...
const AddCarSpawnerForm = ({
  ...
  const [newCarSpawner, setNewCarSpawner] = useState<JobCarSpawnerProps>(
    {} as JobCarSpawnerProps
  );
  ...
  const handleInputChange = (
    ...
    setNewCarSpawner((prevCarSpawner) => {
  ...
  function renderInputField(input: JobInputProps) {
    const uniqueId = `add_carSpawner_${input.uniqueName.replace(".", "_")}`;
    const part = input.uniqueName;
    const currentCarSpawner: any = newCarSpawner[part];
  ...
  return (
    <form onSubmit={(event) => onAddCarSpawner(event, newCarSpawner)}>
      <div className="grid gap-4 mb-2 md:grid-cols-3">
        ...
        <div>
          ...
          {newCarSpawner.cars?.length > 0 && (
            ...
            >
              {newCarSpawner.cars?.map((car, index) => (
                <div
                  key={index}
...
export default AddCarSpawnerForm;
