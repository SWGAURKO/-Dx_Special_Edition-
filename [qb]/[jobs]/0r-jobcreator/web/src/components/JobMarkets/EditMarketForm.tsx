...
import { JobInputProps, JobProps, JobMarketProps } from "../../types/JobTypes";
...
const EditMarketForm = ({
  ...
  Job: JobProps;
  editMarket: JobMarketProps;
  onUpdateMarket: (
    event: React.FormEvent<HTMLFormElement>,
    updatedCarSpawner: JobMarketProps
  ) => void;
}) => {
  const inputs = useMemo(() => AddMarketInputs(Job ?? undefined), [Job]);
  const [market, setMarket] = useState<JobMarketProps>(editMarket);
  ...
  const [marketItemValues, setMarketItemValues] = useState<MarketItemProps>(
    {} as MarketItemProps
  );
  ...
  const handleNewItemValueChange = (
    ...
    const { name, value } = e.target;
    e.preventDefault();
    setMarketItemValues((prevValues) => {
      return {
        ...prevValues,
        [name]: value,
      };
  ...
  const handleAddNewItem = async (
    ...
    event.preventDefault();
    if (
      !marketItemValues.amount ||
      !marketItemValues.item_name ||
      !marketItemValues.label ||
      !marketItemValues.money_type ||
      !marketItemValues.price ||
      !marketItemValues.type
    ) {
      return;
    }
    setMarket((prevMarket) => ({
      ...prevMarket,
      items: [...(prevMarket.items || []), marketItemValues],
    }));
  };

  function renderInputField(input: JobInputProps) {
    const uniqueId = `edit_market_${input.uniqueName.replace(".", "_")}`;
    const part = input.uniqueName;
    const currentMarket: any = market[part];
  ...
  return (
    <form onSubmit={(event) => onUpdateMarket(event, market)}>
      <div className="grid gap-4 mb-2 md:grid-cols-3">
        ...
        <div>
          ...
          {market.ped_coords?.length > 0 && (
            ...
            >
              {market.ped_coords?.map((coords, index) => (
                <div
                  key={index}
        ...
        <div>
          ...
          <div className="w-full flex flex-col bg-primary-700 p-1 gap-1">
            {market.items?.length > 0 && (
              <div
                key="newStep.coords.map"
                className="mb-1 rounded-b bg-primary-900 border border-primary-300 block p-1 text-primary-100 space-y-1"
              >
            ...
            <div>
              <input
                ...
                value={marketItemValues.item_name}
              />
            </div>
            <div>
              <input
            ...
            <div>
              <select
                ...
                name="money_type"
                onChange={handleNewItemValueChange}
                value={marketItemValues.money_type}
              >
