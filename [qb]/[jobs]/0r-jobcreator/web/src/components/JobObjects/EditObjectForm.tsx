...
import { JobInputProps, JobObjectProps } from "../../types/JobTypes";
import { useUtils } from "../../hooks/useUtils";
import { AddObjectInputs } from "./inputs";

const EditObjectForm = ({
  editObject,
  onUpdateObject,
}: {
  editObject: JobObjectProps;
  onUpdateObject: (
    event: React.FormEvent<HTMLFormElement>,
    updatedObject: JobObjectProps
  ) => void;
}) => {
  const inputs = AddObjectInputs;
  const [object, setObject] = useState<JobObjectProps>(editObject);
  ...
  const handleInputChange = (
    ...
    setObject((prevObject) => {
      ...
      let currentSection = updatedObject;
  ...
  function renderInputField(input: JobInputProps) {
    const uniqueId = `add_object_${input.uniqueName.replace(".", "_")}`;
    const part = input.uniqueName;
    const currentObject: any = object[part];
  ...
  return (
    <form onSubmit={(event) => onUpdateObject(event, object)}>
