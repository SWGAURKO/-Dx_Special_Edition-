...
const EditStashForm = ({
  ...
  Job: JobProps;
  editStash: JobStashProps;
  onUpdateStash: (
    event: React.FormEvent<HTMLFormElement>,
    updatedCarSpawner: JobStashProps
  ) => void;
}) => {
  const inputs = useMemo(() => AddStashInputs(Job ?? undefined), [Job]);
  const [stash, setStash] = useState<JobStashProps>(editStash);
  ...
  function renderInputField(input: JobInputProps) {
    const uniqueId = `edit_stash_${input.uniqueName.replace(".", "_")}`;
    const part = input.uniqueName;
    const currentStash: any = stash[part];
  ...
  return (
    <form onSubmit={(event) => onUpdateStash(event, stash)}>
