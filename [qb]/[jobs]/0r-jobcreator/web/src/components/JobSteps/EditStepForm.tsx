...
const EditStepForm = ({
  ...
  const [step, setStep] = useState<JobStepProps>(editStep);
  ...
  function renderInputField(input: JobInputProps) {
    const uniqueId = `edit_step_${input.uniqueName.replace(".", "_")}`;
    const pathParts = input.uniqueName.split(".");
    let currentStep: any = step;
    ...
    if (input.showWhen && !input.showWhen(step ?? ({} as JobStepProps))) {
      return null;
    }
  ...
  return (
    <>
      <form onSubmit={(event) => onUpdateJobStep(event, step)}>
        <div className="grid md:grid-cols-3 gap-4 mb-2">
          <div>
            ...
            <div className="grid grid-cols-3 w-full">
              <input
                className="disabled:bg-primary-700 bg-primary-900 border border-primary-300 text-xs focus:outline-none focus:ring-0 focus:border-primary-400 block p-2.5 text-center text-primary-100"
                type="number"
                step={0.01}
                name="x"
                placeholder="X"
              ...
              />
              <input
                className="disabled:bg-primary-700 bg-primary-900 border border-l-0 border-primary-300 text-xs focus:outline-none focus:ring-0 focus:border-primary-400 block p-2.5 text-center text-primary-100"
                type="number"
                step={0.01}
                name="y"
                placeholder="Y"
