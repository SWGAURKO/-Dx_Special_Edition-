...
RegisterServerEvent('vrp-rahaauto:server:coolout', function()
    CoolDown = true
...
QBCore.Functions.CreateCallback("vrp-rahaauto:server:coolc",function(source, cb)
    if CoolDown then
        cb(true)
    else
        cb(false) 
...
QBCore.Functions.CreateCallback('vrp-rahaauto:server:hasItem', function(source, cb)
    local src = source
...
RegisterNetEvent('vrp-rahaauto:server:giveitem', function ()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('vrp-rahaauto:server:Payouts', function()
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
...
RegisterServerEvent('vrp-rahaauto:server:DeliveryPayouts', function()
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateUseableItem("gps-device", function(source, item)
    ...
    if Player.Functions.RemoveItem(item.name, 1) then
        TriggerClientEvent("vrp-rahaauto:client:gettruck", src, item.name)
    end
end)

QBCore.Functions.CreateUseableItem("kthermite", function(source, item)
    local src = source
    TriggerClientEvent("vrp-rahaauto:client:usethermite", src, item.name)
end)
