...
CreateThread(function()
    local function DrawMain()
        if WarMenu.Button("Toplam:", tostring(ST.Scoreboard:GetPlayerCount()), {r = 135, g = 206, b = 250, a = 150}) then end
        ...
        if WarMenu.MenuButton("Bağlantısı Kesilenler", "recent", {r = 0, g = 0, b = 0, a = 150}) then
        end
    end
    ...
    local function DrawOptions()
        ...
        if WarMenu.Button("Hex ID:", ST._Scoreboard.SelectedPlayer.comid) then end
        if WarMenu.Button("Sunucu ID:", ST._Scoreboard.SelectedPlayer.src) then end
    end
    ...
    local function Init()
        WarMenu.CreateMenu("scoreboard", "Oyuncular")
        WarMenu.SetSubTitle("scoreboard", "Kontrol Et")
        ...
        WarMenu.CreateSubMenu("recent", "scoreboard", "Son B/K")
        ...
        WarMenu.CreateSubMenu("options", "scoreboard", "Oyuncu Bilgisi")
    ...
    while true do
        ...
        if IsControlPressed(0, 303) then 
            if not IsAnyMenuOpen() then
                ...
                TriggerEvent('animations:client:EmoteCommandStartLX', {"düşün2"})
            end
        else
            if IsAnyMenuOpen() then
                ST.Scoreboard.Menu:Close()
                TriggerEvent('animations:client:EmoteCommandStartLX', {"c"})
            end
            Wait(100)
        end
    end
