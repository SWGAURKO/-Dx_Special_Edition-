AddEventHandler('qb-playerDied', function(weaponLabel, time)
    TriggerClientEvent('Dx-deathscreen:openUI', source, time)
end)
...
function OpenDeathScreen(time)
    TriggerClientEvent('Dx-deathscreen:openUI', -1, time) -- -1 sends the event to all clients
end

-- Function to revive a player
function RevivePlayer()
    TriggerClientEvent('Dx-deathscreen:revive', -1)
end

-- Function to update the respawn timer
function UpdateRespawnTimer(time)
    TriggerClientEvent('Dx-deathscreen:updateRes', -1, time)
end
